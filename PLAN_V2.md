...
- **UI redesign STARTED (using reference screenshot #3 — Relatel/microdose light style).** Color: light neutral base + near-black primary + indigo-violet accent. Done so far: postcss fix (Tailwind now compiles), light theme tokens (globals.css), new AppShell layout (grouped sidebar nav + topbar w/ agent status control), `AgentStatusControl` component, **Contact Center console** (`/contact-center`: live SSE incoming-call popup, answer→screen-pop Customer 360, wrap-up disposition codes, recording-consent notice, complete + raise-ticket, my-recent-calls, queue count). Other inner pages still on old dark-slate styling — restyle pending per phase.
- Then **Phase 4** finish executive UI across all pages + dark-mode toggle.
